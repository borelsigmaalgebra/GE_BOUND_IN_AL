...
plot_data <- data.frame(
  ...
  RS = sum_upper_bounds_rs[-c(1,13)],
  
  SQBCR_REG_SUB_4 = sum_upper_bounds_sqbcr_reg_4[-c(1,13)] ,
  SQBCR_REG_SUB_10 = sum_upper_bounds_sqbcr_reg_10[-c(1,13)] ,
  SQBCR_REG_SUB_25 = sum_upper_bounds_sqbcr_reg_25 [-c(1,13)],
  
  SQBCR_AREG_SUB_4 = sum_upper_bounds_sqbcr_areg_4[-c(1,13)] ,
  SQBCR_AREG_SUB_10 = sum_upper_bounds_sqbcr_areg_10[-c(1,13)] ,
  SQBCR_AREG_SUB_25 = sum_upper_bounds_sqbcr_areg_25[-c(1,13)] ,
  
  QBCR_AREG_SUB_4 = sum_upper_bounds_qbcr_areg_4[-c(1,13)] ,
  QBCR_AREG_SUB_10 = sum_upper_bounds_qbcr_areg_10[-c(1,13)] ,
  QBCR_AREG_SUB_25 = sum_upper_bounds_qbcr_areg_25[-c(1,13)] ,
  
  QBCR_REG_SUB_4 = sum_upper_bounds_qbcr_4[-c(1,13)] ,
  QBCR_REG_SUB_10 = sum_upper_bounds_qbcr_10[-c(1,13)] ,
  QBCR_REG_SUB_25 = sum_upper_bounds_qbcr_25[-c(1,13)]
)
...
# Create the ggplot
plot_5 <- ggplot(plot_data_long, aes(x = n_training_data, y = Upper_Bound, color = Method, shape = Method)) +
  geom_line(size = 0.8) +
  geom_point(size = 2.5) +  # Adjust point size for better visibility
  labs(
    x = "No. of Iteration",
    y = "shifted-log upper bound",
    title = dataset_name
  ) +
  scale_y_continuous(
    limits = c(
      min(plot_data_long$Upper_Bound, na.rm = TRUE),
      max(plot_data_long$Upper_Bound, na.rm = TRUE)
    )
  ) +
